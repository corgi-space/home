...
function isUrl(options: string | requestOptions) {
	return isString(options) ? { url: options } : options
}

export function Get<T = unknown>(options: string | requestOptions): Promise<T> {
	return service({
		...isUrl(options),
		method: "get"
	})
...
export function Post<T = unknown>(
	...
	return service({ ...isUrl(options), method: "post" })
}
